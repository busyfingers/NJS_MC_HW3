    ...
    return new Promise((resolve, reject) => {
        ...
        if (data.method == "get") {
            ...
            var templateData = {
                ...
                'body.class': 'ordersAll'
            };
            
            // Get the template with header and footer
            helpers.getCompletePage("ordersAll", templateData).then(page => {
                resolve(response(200, page, "html"));
            }).catch(err => {
                reject(err);
            });
        ...
        if (id && items) {
            ...
            handlers.validateOnResource("carts", id, token).then(_cartData => {
                ...
                Object.keys(items).forEach(key => {
                    let newQuantity = items[key];
                    if (newQuantity === 0 && cartData.items[key] !== undefined) {
                        // Remove item from cart if quantity is changed to 0
                        delete cartData.items[key];
                    } else if (newQuantity > 0) {
                        // Add item(s) to cart if the quantity is > 0, both quantity and price
                        let amount = parseInt(_data.menu[key].replace("$", "")) * newQuantity;
                        cartData.items[key] = { "quantity": newQuantity, "amount": `$${amount}` };
                        totalAmount += amount;
                    }
                    // (do not add 0-quantity items to the cart)
                });
    ...
    return new Promise((resolve, reject) => {
        if (cartId) {
            ...
            handlers.validateOnResource("carts", cartId, token).then(_cartData => {
                ...
                // Process payment
                return helpers.sendPayment(orderData.totalAmount, orderData.email);
            }).then(_ => {
